...
Rijndael_GF Rijndael_GF::get_inverse()
{
	...
	if (exp_lookup[0] != 1)
	{
		ifstream lookup_tables("exp_log_lookup_table.bin", ifstream::binary);
		if (lookup_tables.is_open())
		{
			char temporary[256];
			lookup_tables.read(temporary, 256);
			for (int i = 0; i < 256; i++)
				exp_lookup[i] = unsigned(temporary[i]);
			lookup_tables.read(temporary, 256);
			for (int i = 0; i < 256; i++)
				log_lookup[i] = unsigned(temporary[i]);
		}
		else
			generate_exp_log();
	}
...
void Rijndael_GF::generate_exp_log()
{
	...
	ofstream print_to("exp_log_lookup_table.bin", ofstream::binary);
	print_to.write((char *)exp_lookup, 256);
	print_to.write((char *)log_lookup, 256);
	print_to.close();
...
void Rijndael_GF::generate_inverse()
{
	...
	ofstream print_to("inverse_lookup_table.bin", ofstream::binary);
	print_to.write((char *)inverse_lookup, 256);
	print_to.close();
}
